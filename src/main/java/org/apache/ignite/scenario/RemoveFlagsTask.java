 ...
 * Activates cluster.
 */
public class RemoveFlagsTask extends AbstractTask {
    /** */
    private static final Logger LOG = LogManager.getLogger(RemoveFlagsTask.class.getName());
     ...
     */
    public RemoveFlagsTask(PocTesterArguments args, TaskProperties props){
        super(args, props);
    }
    ...
    @Override public void body0() throws TestFailedException {
        ...
        String[] keys = new String[]{"dataLost", "pauseTX", "pauseRestart", "pauseCheck"};

        for (String key : keys) {
            LOG.info(String.format("Removing key %s from the msgCache", key));

            msgCache.remove(key);

            LOG.info(String.format("Value for key %s in the msgCache is %s", key, msgCache.get(key)));
        }
    }
