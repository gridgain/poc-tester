...
import org.apache.ignite.scenario.internal.NodeStartListener;
...
import org.apache.ignite.scenario.internal.utils.CleanWorker;
import org.apache.ignite.scenario.internal.utils.NodeInfo;
import org.apache.ignite.scenario.internal.utils.NodeResumer;
import org.apache.ignite.scenario.internal.utils.NodeStopper;
...
public class CheckRestartWithEmptyPDSTaskOld extends RestartTaskOld {
    /** */
    private static final Logger LOG = LogManager.getLogger(CheckRestartWithEmptyPDSTaskOld.class.getName());

    public CheckRestartWithEmptyPDSTaskOld(PocTesterArguments args) {
        super(args);
    }

    public CheckRestartWithEmptyPDSTaskOld(PocTesterArguments args, TaskProperties props) {
        super(args, props);
    }
    ...
    @Override public void body0() throws TestFailedException {
        ...
        try {
            toKillList = randomNodes(2, NodeType.SERVER);
        }
        catch (Exception e) {
        ...
        NodeInfo secondToKill = toKillList.get(1);
        ...
        LOG.info(String.format("Trying to slow down started node %s to kill node %s while there is rebalance " +
            "in progress on the started node", startedNode.getNodeConsId(), secondToKill.getNodeConsId()));
        ...
        Queue<NodeInfo> secondStoppedNode = stopNodes(Collections.singletonList(secondToKill));
        ...
        NodeStartListener lsnr2 = new NodeStartListener(secondStoppedNode);

        ignite().events().localListen(lsnr2, EventType.EVT_NODE_JOINED);

        waitForStart = true;

        startNodes(secondStoppedNode);

        try {
            if (!lsnr2.waitForEvent())
        ...
        try {
            waitForStringInLog(startedNode, "\\[main\\] Topology snapshot");
        }
        catch (Exception e) {
            LOG.error(String.format("Failed to wait for string 'Topology snapshot' from the node %s",
                startedNode.getNodeConsId()), e);
        }

//        try {
//            if (!lsnr1.waitForEvent())
//                LOG.error("Failed to wait for join events for restarted nodes.");
//            else
//                waitForRebalance(lsnr1.getNodeIds(), lsnr1.getTopVer() - 1);
//        }
//        catch (InterruptedException e) {
//            LOG.error("Failed to wait for join events for restarted nodes.", e);
//        }
//
//
        long newTotalSize = getTotalCachesSize();

        if(totalSize != newTotalSize)
            LOG.error(String.format("Total size after iteration %s does not equal total size before %d",
                newTotalSize, totalSize));
        else
            LOG.info(String.format("Total size after iteration %s is equal total size before %d",
                newTotalSize, totalSize));
